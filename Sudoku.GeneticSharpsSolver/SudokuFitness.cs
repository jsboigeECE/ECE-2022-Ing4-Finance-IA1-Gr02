...
namespace Sudoku.GeneticSharpsSolver
{
    public class SudokuFitness : IFitness 
    {
        ...
        public double Evaluate(GridSudoku testGridSudoku)
        {
            ...
            var cells = testGridSudoku.Cells.Select((c, i) => new { index = i, cell = c }).ToList();
            ...
            toReturn -= cells.Count(x => _targetGridSudoku.Cells[x.index] > 0 && _targetGridSudoku.Cells[x.index] != x.cell); // Mask
            return toReturn;
        }
    }
}
